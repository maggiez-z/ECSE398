...
Uint16 first=0;              // Count and Dly captured by 1st RISE/FALL event after cal, soft reset, or clock enable
...
volatile Uint32 highpulse[100];  // Stores 100 high pulses in # of HCCAPCLK cycles (Q16 format: int + fraction)
volatile Uint16 array_index = 0;
volatile Uint16 pulse_array_updated = 0;
volatile Uint32 pulseWidth1, pulseWidth0;
...
volatile Uint16 save_ovflow_count = 0;
...
//    #endif
	HRCap2Regs.HCCTL.bit.OVFINTE = 1;	   // Enable Interrupt on 16-bit Counter Overflow Event
...
__interrupt void HRCAP2_Isr (void)
{
	...
	if (first < 1) {
		...
		if (HRCap2Regs.HCIFR.bit.RISEOVF == 1) {
			...
			HRCap2Regs.HCICLR.all = 0xFFFF;        // Clear all interrupt flags (RISEOVF, COUNTEROVF, FALL, RISE, INT)
	    } else if (start_cap) // Only read the HRCAP input when start_cap is set.
		{
			highpulse[array_index] = PeriodWidthRise0((Uint16 *)&HRCap2Regs);
            if (HRCap2Regs.HCIFR.bit.COUNTEROVF == 1)
			{
				overflow_count++;
				new_overflow_flag = 1;
				HRCap2Regs.HCICLR.bit.COUNTEROVF = 1; // Clear CounterOVF flag
			} else if (HRCap2Regs.HCIFR.bit.RISE == 1)
			{
				if (overflow_count <= 10)
				{
					if ((new_overflow_flag == 0) && (highpulse[array_index] <= 0x4B00000)) // if <= 10us make it overflow
						overflow_count++;
					...
					//start_cap = 0;
					save_ovflow_count = overflow_count;
				}
				overflow_count = 0;
				new_overflow_flag = 0;
			...
			HRCap2Regs.HCICLR.all = 0xFFFF;        // Clear all interrupt flags except COUNTEROVF
	}
    HRCap2Regs.HCICLR.all = 0xFFF5;        // Clear all interrupt flags except RISE & COUNTEROVF (handled above separately)
	PieCtrlRegs.PIEACK.bit.ACK4=1; // Acknowledge PIE Group 4 interrupts.
	EDIS;
}
/*
// 5/15/14 test if TDC1000 based RTD measure will work with C2000
...
void InitSysCtrl_ExtOsc(void)
{
   ...
   // Initialize the PLL control: PLLCR and CLKINDIV
   // DSP28_PLLCR and DSP28_CLKINDIV are defined in DSP2803x_Examples.h
   // Note: if board has 12MHz crystal, change it to 10 to get 120MHz, use 6 for 20MHz crystal, 15 for 8MHz ext osc
   // InitPll(6,DSP28_DIVSEL);		// 20 MHz crystal
   // InitPll(15, DSP28_DIVSEL);    // 8MHz crystal, 60MHz operation, Operation out of specs
      InitPll(7,DSP28_DIVSEL);		// 16 MHz crystal, 56MHz operation
   // Initialize the peripheral clocks
   InitPeripheralClocks();
}
